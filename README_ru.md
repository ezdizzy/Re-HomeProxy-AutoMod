...
- **MultiDNS (движок mosdns)** — ускоритель DNS: каждый запрос гоняется параллельно по всем серверам пула (обычный
  «Россия» и шифрованный «Secure»), возвращается самый быстрый живой ответ. Отдельный демон качества проверяет по HTTPS,
  что возвращённые серверами IP реально открывают сайты, и автоматически исключает (prune) мёртвые/отравленные серверы и
  IP. Настройка — вкладка **MultiDNS** в «Настройках клиента», установка/обновление mosdns — карточка в **«Ядро и службы»**.
...
- **Интерактивный установщик** — `install.sh` ведёт установку по шагам (подписка, MultiDNS, Автоматизация, Zapret с
...
не будут), добавление подписки или строки подключения, включение **MultiDNS** и **Автоматизации**, установку
**Zapret 2** с автотестом стратегий (начинает с Hostfakesplit и проверяет YouTube) и **ByeDPI** (на ваше усмотрение).
...
выбрать основной узел, вкл/выкл MultiDNS и Автоматизацию, подобрать стратегию Zapret, обновить ядро и т.д. Работает на APK
