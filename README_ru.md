...
- **Вкладка «Автоматизация»** — автоматическое обнаружение заблокированных сайтов. Фоновый монитор проверяет узлы и
  ...
  conntrack-адреса проверяются TCP-пробой через оба пути; общие CDN-сети исключены, выученные записи самолечятся
  (разблокированный сайт сам возвращается на прямой путь). Выученный список применяется hot-reload'ом — без рестарта
  и без разрыва соединений.
...
- **URLTest с тремя режимами** — автовыбор узла: **Авто** (все импортированные узлы), **Приоритетный узел + авто**
  ...
  (классический выбор узлов). Интервал проверки и допуск настраиваются (по умолчанию 30 с / 150 мс); узлы, которые
  активное ядро не поддерживает (например, MIERU на hiddify-core), в пул не попадают — мёртвый узел больше не «залипает».
