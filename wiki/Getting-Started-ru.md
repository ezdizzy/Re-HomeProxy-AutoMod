...
Эта страница проведёт вас от свежей установки до рабочего соединения. Предполагается, что Re:HomeProxy AutoMod уже установлен (установку пакетов см. в [README](../../blob/master/README_ru.md)).

> Всё ниже делается в веб-интерфейсе LuCI: **Сервисы → Re:HomeProxy AutoMod**.
...
- **hiddify-core** (по умолчанию) — легче, есть компактная сборка для маленьких роутеров; единственное ядро с **NaïveProxy**.
- **sing-box-extended** — более новая база sing-box 1.14 (WARP / MASQUE / TrustTunnel в ядре); без NaïveProxy — такие узлы пропускаются автоматически.
...
На странице **Настройки клиента** задайте **Основной узел** = добавленный узел — или выберите **URLTest**, чтобы Re:HomeProxy AutoMod автоматически выбирал самый быстрый доступный узел и переключался при сбоях.
