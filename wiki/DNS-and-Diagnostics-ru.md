...
Две связанные темы: как Re:HomeProxy AutoMod резолвит имена (и почему это важно для утечек) и как проверить, что всё работает.

---

## DNS

Пулы DNS, резервный DNS и MultiDNS настраиваются на отдельной странице **[Настройки DNS](DNS-Settings-ru)** (Службы → Re:HomeProxy AutoMod → Настройки DNS). В **Настройки клиента → Настройки маршрутизации** остался только одиночный DNS-сервер Global-режима (с пресетом и альтернативными серверами). Какие поля видны, зависит от режима маршрутизации.
...
Страница **Диагностика** (Сервисы → Re:HomeProxy AutoMod → Диагностика) запускает живые проверки. У каждой карточки своя кнопка **Проверить**.
