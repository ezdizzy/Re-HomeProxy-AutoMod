...
Есть четыре способа добавить узлы в Re:HomeProxy AutoMod. Все они — на **Настройки узлов → Узлы** (кнопки импорта) и **Настройки узлов → Подписки**.
...
Содержимое подписок и share-ссылок очень часто закодировано в **base64** — это норма, а не редкий случай. Re:HomeProxy AutoMod декодирует base64 прозрачно:
...
На вкладке **Подписки** добавьте одну или несколько **ссылок подписок**. Роутер скачивает их и поддерживает список узлов в актуальном состоянии. Re:HomeProxy AutoMod автоматически распознаёт несколько форматов ответа:
