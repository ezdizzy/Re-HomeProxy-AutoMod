...
Работает в **обоих** ядрах: приложение формирует WireGuard-эндпоинт с вложенными опциями `amnezia`, которые принимают и hiddify-core (4.1.0+), и sing-box-extended.

> ⚠️ **Обратная сторона — NaïveProxy:** он собран в hiddify-core, но **отсутствует** в текущой сборке sing-box-extended (нет флага `with_naive_outbound`). Приложение определяет это по тегам сборки ядра и при работе sing-box-extended **автоматически пропускает NaïveProxy-узлы** — они просто исчезают из пулов, а интерфейс показывает, какие узлы были пропущены.

---

## TrustTunnel — требуется ядро sing-box-extended

### TrustTunnel
Обфусцированный VPN-протокол AdGuard: туннель неотличим от обычного HTTPS-трафика. Поддерживает релей TCP и UDP, QUIC-транспорт, выбор контроля перегрузки (BBR / BBRv2 / New Reno) и мультиплексирование потоков. Требует флаг сборки `with_trusttunnel` — есть в sing-box-extended, отсутствует в hiddify-core, поэтому **узлы этого типа автоматически пропускаются при работе hiddify-core** (из подписки они импортируются и остаются в списке узлов).
