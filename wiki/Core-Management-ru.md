...
Re:HomeProxy AutoMod **многоядерный**: приложение LuCI — это интерфейс, а само проксирование выполняет отдельный бинарник-**ядро**. Установка, обновление и переключение ядер выполняются на вкладке **Сервисы → Re:HomeProxy AutoMod → Ядро и службы → Управление ядром** — без SSH.
...
|---|---|---|
| Движок | Форк sing-box 1.13 от команды Hiddify | Расширенная база sing-box 1.14 (shtorm-7) |
| Размер | Легче; есть **компактная сборка** для слабых устройств | Крупнее (~26 МБ установленного) |
| Протоколы приложения | Все протоколы, которые умеет настраивать приложение | Все протоколы, которые умеет настраивать приложение |
| **NaïveProxy** | ✅ **Поддерживается** | ❌ **Нет** (убран из этой сборки — такие узлы **пропускаются автоматически**) |
| **AmneziaWG** | ✅ Поддерживается | ✅ Поддерживается |
| **WARP / MASQUE / TrustTunnel** | ❌ Приложением не используются | ✅ Есть в ядре (серверные/эндпоинт-возможности; в интерфейсе приложения пока не представлены) |

**Правило:** оба ядра работают со всеми протоколами, которые умеет настраивать приложение — практическая разница в **NaïveProxy** (только hiddify-core) и в более новой базе sing-box у extended-ядра. Если в подписке есть NaïveProxy-узлы — оставайтесь на **hiddify-core**. При переключении на **sing-box-extended** неподдерживаемые узлы автоматически исключаются из всех пулов (приложение покажет какие). hiddify-core — также более лёгкий вариант по умолчанию и единственный с компактной сборкой для роутеров с малым объёмом памяти.
...
- **Переключение ядер:** установите другое ядро; если присутствуют оба, Re:HomeProxy AutoMod использует настройку **Предпочтительное ядро** (на вкладке **Настройки клиента → Настройки маршрутизации**). Генератор конфигурации и служба учитывают ту же настройку, поэтому сгенерированный конфиг всегда соответствует ядру, которое его запустит.
- **Своё / внешнее ядро:** вместо управляемой установки можно указать Re:HomeProxy AutoMod на **собственный бинарник ядра** (свою сборку или версию, которой нет в Releases) — он его обнаружит и использует. Только для продвинутых сценариев.
